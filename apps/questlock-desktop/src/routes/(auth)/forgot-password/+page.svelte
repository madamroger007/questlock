<script lang="ts">
  ...
  async function handleForgotPassword() {
    ...
    try {
      ...
      errorMessage =
        err.message || "Gagal menguraikan instruksi reset password.";
    } finally {
      loading = false;
    }
  }
...
<AuthLayout
  title="Lupa Password"
  subtitle="Masukkan alamat email Anda untuk menerima instruksi pemulihan akun"
>
  <AuthCard>
    {#if errorMessage}
      <div
    ...
    {#if successMessage}
      <div
        class="mb-4 p-3 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs sm:text-sm rounded-xl text-center"
      >
        {successMessage}
      </div>
    {/if}
    ...
    <p class="mt-6 text-center text-xs text-slate-400">
      <a href="/login" class="text-sky-400 font-semibold hover:underline"
        >Kembali ke Sign In</a
      >
    </p>
  </AuthCard>
</AuthLayout>
